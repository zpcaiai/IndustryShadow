...
def main() -> int:
    ...
    admin_store.execute(
        f"CREATE ROLE {role} LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS PASSWORD '{password}'"
    )
    admin_store.execute(f"GRANT USAGE ON SCHEMA public TO {role}")
    admin_store.execute(
    ...
    store = open_store(role_url, ROOT / "migrations", migrate=False)
    first = ActorContext(
        "postgres-validator", "tenant-a", f"workspace-a-{suffix}", frozenset({"Admin"})
    )
    second = ActorContext(
        "postgres-validator", "tenant-b", f"workspace-b-{suffix}", frozenset({"Admin"})
    )
    ...
    with workspace_scope(first.workspace_id):
        created = repository.create(first, "migration_probe", f"probe-{suffix}", model)
        if (
            repository.get(first, "migration_probe", created.resource_id).digest
            != created.digest
        ):
            raise SystemExit("PostgreSQL resource round trip failed")
        first_run = RunOrchestrator(store).create(first, manifest, "shared-key")
    with workspace_scope(second.workspace_id):
        ...
        second_run = RunOrchestrator(store).create(second, manifest, "shared-key")
        if first_run["run_id"] == second_run["run_id"]:
            raise SystemExit("idempotency key leaked across workspaces")
        if RunOrchestrator(store).create(second, manifest, "shared-key") != second_run:
            raise SystemExit("workspace-scoped idempotency replay failed")
    if store.query("SELECT * FROM domain_resources"):
        raise SystemExit("unscoped PostgreSQL role bypassed row-level security")
    if store.query("SELECT * FROM runs") or store.query("SELECT * FROM outbox"):
        raise SystemExit("unscoped PostgreSQL role exposed run or outbox records")
    rows = admin_store.query("SELECT version FROM schema_migrations ORDER BY version")
